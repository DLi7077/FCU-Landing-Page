let intital = 500; //A1
const interest = 0.1; //A2
const interest_compound_rate = 365; //A3 once per year (annual)
//annual =1
//daily= 365
//biyearly = 0.5
const time = 4; //how many years it will be compounded

const result =
  intital * (1 + interest / interest_compound_rate) ** (365 * time);

console.log(result);

const monthy_contribution = 10;

const monthly =
  (10 *
    (1 + interest / interest_compound_rate) ** (interest_compound_rate * time) -
    1) /
  (interest / interest_compound_rate);
...
function validateFormLoanAfford() {
  ...
  return (
    ...
    (n = temparray.join("")),
    isNaN(e)
      ? (alert("Amount must be a number!"), $("#loanAffordAmount").focus(), !1)
      : isNaN(t)
      ? (alert("Payment must be a number!"),
        $("#loanAffordPayment").focus(),
        !1)
      : isNaN(a)
      ? (alert("Rate must be a number!"), $("#loanAffordRate").focus(), !1)
      : isNaN(n)
      ...
      : 0 != e && "" != e && 0 != t && "" != t
      ? (alert(
          "Please clear either the loan payment or the loan amount to continue."
        ),
        ...
        : ((a /= 1200),
          0 == n || "" == n
            ? (alert("You must provide the term of the loan!"),
              $("#loanAffordMonths").focus(),
              !1)
            : ("1" == $("#loanAffordFrequency").val() && (n *= 12),
              0 == t || "" == t
                ? $("#loanAffordPayment").val(
                    parseInt(
                      100 * (e * (a / (1 - Math.pow(1 + a, -n))) + 0.005)
                    ) / 100
                  )
                : $("#loanAffordAmount").val(
                    parseInt(
                      100 *
                        ((Math.pow(1 + a, -n) * (-t + Math.pow(1 + a, n) * t)) /
                          a +
                          0.005)
                    ) / 100
                  ),
...
function validateFormLoanPayment() {
  ...
  return (
    ...
    (n = temparray.join("")),
    isNaN(e)
      ? (alert("Amount must be a number!"), $("#loanPaymentAmount").focus(), !1)
      : isNaN(t)
      ? (alert("Payment must be a number!"),
        $("#loanPaymentPayment").focus(),
        !1)
      : isNaN(a)
      ? (alert("Rate must be a number!"), $("#loanPaymentRate").focus(), !1)
      : isNaN(n)
      ? (alert("Length of Loan must be a number!"),
        ...
        : ((a /= 1200),
          0 == n || "" == n
            ? (alert("You must provide the term of the loan!"),
              $("#loanPaymentMonths").focus(),
              !1)
            : ("1" == $("#loanPaymentFrequency").val() && (n *= 12),
              0 == t || "" == t
                ? $("#loanPaymentPayment").val(
                    parseInt(
                      100 * (e * (a / (1 - Math.pow(1 + a, -n))) + 0.005)
                    ) / 100
                  )
...
function singleDepositComputeForm() {
  if (
    !(
      isNaN($("#singleDepositPrincipal").val()) ||
      isNaN($("#singleDepositInterest").val()) ||
      isNaN($("#singleDepositPayments").val())
    )
  ) {
    ...
    if (1 > e > 480) alert("Years to Accrue must be between 1 and 480");
      ...
      alert("Interest rate must be between 0.001 and 99");
...
function multipleDepositComputeForm() {
  if (
    (console.log("!!"),
    !(
      isNaN($("#multipleDepositMoAdd").val()) ||
      isNaN($("#multipleDepositInterest").val()) ||
      isNaN($("#multipleDepositPayments").val())
    ))
  )
    if ("" != $("#multipleDepositInterest").val())
      if ("" != $("#multipleDepositMoAdd").val())
        if ("" != $("#multipleDepositMoAdd").val()) {
          var e = parseInt(
              $("#multipleDepositPayments").val().replace(",", "")
            ),
            t = parseFloat(
              $("#multipleDepositInterest").val().replace(",", "")
            ),
            a = parseFloat(
              $("#multipleDepositPrincipal").val().replace(",", "")
            ),
            n = parseFloat($("#multipleDepositMoAdd").val().replace(",", "")),
            o = t;
          (o = o >= 1 ? t / 100 : t), (o /= 12);
          for (var r = n, l = a, i = 12 * e, s = 0; s < i; )
            (newprin = l + r), (l = newprin * o + (l + r)), (s += 1);
  ...
  if ("" != $("#mortgagePaymentPayment").val())
    if ("" != $("#mortgagePaymentPrincipal").val())
      if ("" != $("#mortgagePaymentIntRate").val()) {
        ...
        u > 1 && (u /= 100);
        for (
          var p = u / 12, f = u / 26, g = 0, c = 0;
          a > 0 && ((a -= l = e - (o = a * p)), l, (s += o), !((g += 1) > 600));

        );
        for (
          $("#mortgagePaymentOrigInt").text("$" + s.formatMoney(2, ".", ","));
          n > 0 &&
          ((n -= i = t - (r = n * f)), i, (m += r), (c += 1), !(g > 600));
        ...
        $("#mortgagePaymentIntSave").text("$" + y.formatMoney(2, ".", ",")),
          $("#mortgagePaymentTime").text(
            "By paying bi-weekly, you are adding a 13th payment to your annual number of payments, and splitting it up between 26 bi-weekly payments. By paying an extra $" +
              parseFloat(e / 26).formatMoney(2, ".", ",") +
              " every two weeks, you will pay off your mortgage in " +
              parseInt((c / 26) * 12, 10) +
              " months instead of the current " +
              g +
              " months, and save $" +
              parseFloat(s - m).formatMoney(2, ".", ",") +
        ...
        if ("" != $("#mortgageRefinanceIntRate2").val())
          if ("" != $("#mortgageRefinanceNper2").val())
            if ("" != $("#mortgageRefinanceClosingCost").val())
              if (
                parseFloat($("#mortgageRefinanceIntRate2").val()) >
                parseFloat($("mortgageRefinanceIntRate").val())
              )
                alert(
                  "You have entered a refinancing rate that is higher than your present rate. The refinancing rate must be lower than your present rate for this calculator to function."
                );
              else {
                var e = 0,
                  t = 0,
                  ...
                  r = 0,
                  l = 0;
                (i = parseFloat($("#mortgageRefinanceIntRate").val())) > 1 &&
                  (e = i /= 100);
                var i = i / 12;
                (s = parseFloat($("#mortgageRefinanceIntRate2").val())) > 1 &&
                  (t = s /= 100);
                for (
                  var s = s / 12, m = 0;
                  n > 0 &&
                  ((n -= r = a - (o = n * i)), r, (l += o), !((m += 1) > 600));

                );
                $("#mortgageRefinanceOrigInt").text(
                  "$" + l.formatMoney(2, ".", ",")
                );
                for (
                  var u = 1, p = 0;
                  p < 12 * parseInt($("#mortgageRefinanceNper2").val());
                  p++
                )
                  u *= 1 + s;
                var f =
                  (parseFloat($("#mortgageRefinancePrincipal").val()) * u * s) /
                  (u - 1);
                $("#mortgageRefinancePayment2").text(
                  "$" + f.formatMoney(2, ".", ",")
                );
                var g = parseFloat($("#mortgageRefinancePayment").val()) - f;
                $("#mortgageRefinanceMoSave").text(
                  "$" + g.formatMoney(2, ".", ",")
                );
                var c =
                  f * parseInt($("#mortgageRefinanceNper2").val()) * 12 -
                  parseFloat($("#mortgageRefinancePrincipal").val());
                $("#mortgageRefinanceTotInt2").text(
                  "$" + c.formatMoney(2, ".", ",")
                );
                var y = l - c;
                $("#mortgageRefinanceIntSave").text(
                  "$" + y.formatMoney(2, ".", ",")
                ),
                  $("#mortgageRefinanceCloseMo").text(
                    parseInt(
                      parseFloat($("#mortgageRefinanceClosingCost").val()) / g,
                      10
                    )
                  );
                var v =
                  y - parseFloat($("#mortgageRefinanceClosingCost").val());
                $("#mortgageRefinanceNetSave").text(
                  "$" + v.formatMoney(2, ".", ",")
                ),
                  $("#mortgageRefinanceSummary").text(
                    "If you refinance your current " +
                      (100 * e).formatMoney(3, ".", ",") +
                      "% mortgage to a " +
                      (100 * t).formatMoney(3, ".", ",") +
                      "% mortgage, your monthly payment will drop by " +
                      $("#mortgageRefinanceMoSave").text() +
                      " and you will save " +
                      $("#mortgageRefinanceIntSave").text() +
                      " in interest charges over the life of the mortgage.  However, in order for this refinancing to yield any savings at all you will need to stay in your current home for at least " +
                      $("#mortgageRefinanceCloseMo").text() +
                      " months.  That is how long it will take for your monthly payment savings to offset the closing costs attributable to refinancing."
                  );
              }
